...
export default function AdminPanel() {
  ...
  const handleDeleteUser = async (user) => {
    if (!window.confirm(`User "${user.username || user.id.slice(0, 8)}" wirklich komplett löschen?`)) {
      return;
    }
    ...
    try {
      ...
      setStats((prev) => ({
        ...prev,
        users: Math.max(0, (prev.users || 1) - 1),
      }));
      setLatestUsers((prev) => prev.filter((u) => u.id !== user.id));
    } catch (err) {
            ...
            <article className="admin-table-card">
              ...
              <table className="admin-table">
                ...
                <tbody>
                    ...
                    <tr key={u.id}>
                      ...
                      <td>
                        <button
                          type="button"
                          className="btn"
                          style={{ fontSize: '.75rem', paddingInline: '.8rem' }}
                          disabled={deletingId === u.id}
                          onClick={() => handleDeleteUser(u)}
                        >
                          {deletingId === u.id ? 'Lösche…' : 'Löschen'}
                        </button>
                      </td>
                    </tr>
                  ))}
                  {!latestUsers.length && (
                    <tr>
                      <td colSpan={3}>Noch keine Nutzer gefunden.</td>
                    </tr>
                  )}
                </tbody>
              </table>
